...
bool wait_for_status(pid_t pid, int expected_status, int timeout_seconds, bool& saw_running, int& exit_code)
{
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (process_status == 0) {
            ...
            std::cerr << "  Unexpected process_status=" << process_status
...
int t_launch_process()
{
    ...
    if (pid > 0) {
        ...
        if (wait_for_status(pid, 1, 10, saw_running, exit_code) && saw_running && exit_code == 0) {
            ...
            std::cerr << "  Normal exit case did not behave as expected (exit_code="
                      << exit_code << ")\n";
        }
    } else {
    ...
    if (setenv(kSignalEnv, "TERM", 1) != 0) {
        ...
        if (pid > 0) {
            ...
            if (wait_for_status(pid, 3, 10, saw_running, exit_code) && exit_code == -1) {
                ...
                std::cerr << "  Signal termination case did not behave as expected (exit_code="
                          << exit_code << ")\n";
            }
        } else {
    ...
    std::cout << "  launch_process/check_child_status: " << test_passed << "/"
