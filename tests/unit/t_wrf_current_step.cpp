// Unit tests for WRF current-step extraction from stderr timing output.
//
...
int t_wrf_current_step()
{
    ...
    test_count++;
    const std::string stderr_content = " CPDN DEBUG: calling cpdn_checkpid for grid%id = 1\n"
    ...
    if ( !write_text_file( tmp_dir / "stderr.txt", stderr_content ) ) {
        TEST_FAIL;
        std::cout << "Unable to write WRF stderr.txt test file\n";
        fs::current_path( original_cwd );
        fs::remove_all( tmp_dir, ec );
        return EXIT_FAILURE;
    }
    ...
    test_count++;
    if ( !write_text_file( tmp_dir / "stderr.txt", "Timing for main: time 2022-07-01_00:50:05 on domain   3:   11.33282 elapsed seconds\n" ) ) {
        TEST_FAIL;
        std::cout << "Unable to rewrite WRF stderr.txt test file\n";
        fs::current_path( original_cwd );
        fs::remove_all( tmp_dir, ec );
        return EXIT_FAILURE;
    }
