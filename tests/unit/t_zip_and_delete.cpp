...
int t_zip_and_delete()
{
    ...
    test_count++;
    {
        ...
        if ( zip_exists && source_deleted && success ) {
            ...
            if ( !zip_exists )
                std::cerr << "    - Zip file not created\n";
            if ( !source_deleted )
                std::cerr << "    - Source file not deleted\n";
            if ( !success )
                std::cerr << "    - Return value was not 0\n";
        }

        // Cleanup
        if ( fs::exists( zip_output ) )
            fs::remove( zip_output );
    }
    ...
    test_count++;
    {
        ...
        bool all_deleted = ( !fs::exists( test_file1 ) && !fs::exists( test_file2 ) && !fs::exists( test_file3 ) );
        // Check: return value is 0 (success)
        bool success = ( result == 0 );
        ...
        if ( zip_exists && all_deleted && success ) {
            ...
            if ( !zip_exists )
                std::cerr << "    - Zip file not created\n";
            if ( !all_deleted ) {
                if ( fs::exists( test_file1 ) )
                    std::cerr << "    - File 1 not deleted\n";
                if ( fs::exists( test_file2 ) )
                    std::cerr << "    - File 2 not deleted\n";
                if ( fs::exists( test_file3 ) )
                    std::cerr << "    - File 3 not deleted\n";
            }
            if ( !success )
                std::cerr << "    - Return value was not 0\n";
        }
    ...
    test_count++;
    {
        ...
        if ( returned_failure && zip_not_created ) {
            ...
            if ( !returned_failure )
                std::cerr << "    - Should return 1 on failure, got " << result << "\n";
            if ( !zip_not_created )
                std::cerr << "    - Zip file should not be created when source missing\n";
        }
    ...
    test_count++;
    {
        ...
        if ( returned_failure && existing_still_there ) {
            ...
            if ( !returned_failure )
                std::cerr << "    - Should return 1 on failure\n";
            if ( !existing_still_there )
                std::cerr << "    - Existing file should NOT be deleted on zip failure\n";
        }
    ...
    test_count++;
    {
        ...
        if ( first_success && second_success && final_source_deleted ) {
            ...
            if ( !first_success )
                std::cerr << "    - First zip failed\n";
            if ( !second_success )
                std::cerr << "    - Second zip failed\n";
            if ( !final_source_deleted )
                std::cerr << "    - Final source file not deleted\n";
        }
